...
function queryStockHistoryHighAndLowPrice(codeList, args, callback) {
    ...
    var sql = 'select a.code, a.lowIntervalMinPrice, b.highIntervalMaxPrice' +
        ' from (select a.code, min(a.before_rehabilitation_close) as lowIntervalMinPrice from (select code, before_rehabilitation_close from stock_following where code in ' + inCondition.inSql + ' order by date desc limit :lowLimit) as a group by a.code) as a' +
        ' ,(select a.code, max(a.before_rehabilitation_close) as highIntervalMaxPrice from (select code, before_rehabilitation_close from stock_following where code in ' + inCondition.inSql + ' order by date desc limit :highLimit) as a group by a.code) as b' +
        ' where a.code = b.code;';
    ...
    function _convertSinaResult(result) {
        ...
        _.each(codeList, function (code) {
            ...
            code2CurrentPrice[code.substring(2)] = {
                ...
                before_rehabilitation_open: currentInfo[1],
                before_rehabilitation_close: currentInfo[3],
