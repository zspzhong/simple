...
function register(serviceInfo) {
    // 所有注册的服务都加上svc前缀
    var path = '/svc' + serviceInfo.path;
    var realizeAfterWrap = _wrapRealize(serviceInfo.realizeFunction);

    if (!serviceInfo.oauth) {
        global.express[serviceInfo.method](path, realizeAfterWrap);
    }
    else if (serviceInfo.oauth === 'simple-session') {
        global.express[serviceInfo.method](path, [passport.authenticate('simple-session', {})], realizeAfterWrap);
    }
